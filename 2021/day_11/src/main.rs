...
fn step(octopuses: Vec<Vec<i32>>) -> (Vec<Vec<i32>>, usize) {
    ...
    for line in 0..out_octopuses.len() {
        // incrementing every value in the vector
                ...
                for octopus in 0..out_octopuses[line].len() {
                    if out_octopuses[line][octopus] > 9 {
                        ...
                        if line as i32 - 1 >= 0 {
                            ...
                            if octopus + 1 < out_octopuses[line].len() {
                                out_octopuses[line - 1][octopus + 1] =
                                    out_octopuses[line - 1][octopus + 1] + 1;
                            } // top-right
                            if octopus as i32 - 1 >= 0 {
                                out_octopuses[line - 1][octopus - 1] =
                                    out_octopuses[line - 1][octopus - 1] + 1;
                            } // top-left
                        }
                        if octopus as i32 - 1 >= 0 {
                            // left
                            out_octopuses[line][octopus - 1] = out_octopuses[line][octopus - 1] + 1;
                            // left
                        }
                        if octopus + 1 < out_octopuses[line].len() {
                            // right
    ...
    for coord in &coords_of_flashes {
        out_octopuses[coord[0]][coord[1]] = 0;
    }
...
fn part1(input_file: String) {
    ...
    let mut octopuses: Vec<Vec<i32>> = input_file
        .split("\n")
        .map(|c| {
            c.chars()
        ...
        .collect::<Vec<Vec<i32>>>();
    if octopuses.last().unwrap().is_empty() {
        octopuses.pop();
    }
    for _ in 0..100 {
...
fn part2(input_file: String) {
    ...
    for i in 0..1000 {
        octopuses = step(octopuses).0;
        //print_octopuses(&octopuses);
        if octopuses.iter().all(|o| o == &octopuses[0]) {
            answer = i + 1;
            break;
        }
    }
